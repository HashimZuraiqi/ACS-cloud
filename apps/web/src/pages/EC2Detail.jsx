...
const EC2Detail = () => {
    ...
    const fetchScan = async () => {
        try {
            ...
            setError(err.message || "Failed to load EC2 scan");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleRemediationDone = (result) => {
        toast({ title: "EC2 Remediation Applied", description: result.message, variant: "success" });
        if (result.newScanId) {
            navigate(`/ec2/${result.newScanId}`);
        } else {
            fetchScan();
        }
    };
    ...
    if (error || !scanResult) {
        return (
            <div className="flex flex-col items-center justify-center h-[calc(100vh-200px)] text-center p-8">
                ...
                <h2 className="text-xl font-bold text-foreground mb-2">Error Loading EC2 Scan</h2>
    ...
    return (
        <div className="space-y-8 pb-12">
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-8">
                    <div>
                        <div className="flex items-center gap-4 mb-3">
                            <div className="p-2 rounded-xl bg-gradient-to-br from-orange-500 to-amber-600 shadow-lg shadow-orange-600/20">
                                <Server className="w-6 h-6 text-white" />
                            </div>
                            <h1 className="text-3xl font-bold text-foreground">{scanResult.instanceId}</h1>
                        ...
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground">
                            ...
                            {scanResult.configuration.instance_type && (
                                <span>Type: <span className="text-foreground font-medium">{scanResult.configuration.instance_type}</span></span>
                            )}
                            <span>Region: <span className="text-foreground font-medium">{scanResult.configuration.region || 'us-east-1'}</span></span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={2} className="space-y-6">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="p-2 rounded-lg bg-orange-500/10 text-orange-500"><FileJson className="w-5 h-5" /></div>
                        <h2 className="text-lg font-semibold text-foreground">Instance Configuration</h2>
                    </div>
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    {scanResult.remediationPlan?.length > 0 && (
                        <Button
                            ...
                            disabled={scanResult.planStatus === 'NO_ACTION_NEEDED'}
                        ...
                        >
                            {scanResult.planStatus === 'ASSISTED_ONLY' ? "Generate Fix Scripts" : "Approve & Auto Fix"}
                        </Button>
                    )}
                </div>
            ...
            <ApprovalModal
                ...
                resourceName={scanResult.instanceId}
                actionCount={scanResult.remediationPlan?.length || 0}
                serviceType="ec2"
                onDone={handleRemediationDone}
            />
        </div>
    );
};

export default EC2Detail;
